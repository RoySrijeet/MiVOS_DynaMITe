        ...
        for seq in list(dataloader_dict.keys()):
            ...
            while lowest_frame_index!=-1:
                ...
                if out_masks is not None:                                                           # out_masks = [num_frames, H, W]
                    ...
                    pred_masks = np.zeros((seq_num_instances,mask_H,mask_W)).astype('uint8')                        # pred_masks = [num_instances, H, W]
                ...
                if pred_masks.shape[0] != seq_num_instances:
                    dummy = np.zeros((seq_num_instances, pred_masks.shape[1], pred_masks.shape[2])).astype('uint8')
                    j = 0
                    for i in range(seq_num_instances):                        
                        if i+1 in missing_obj_ids:
                            continue
                ...
                bg_mask = np.ones(pred_masks.shape[-2:]).astype('uint8')                
                for i in range(seq_num_instances):
                    bg_mask[np.where(pred_masks[i]==1.)]=0   
                bg_mask = torch.from_numpy(bg_mask).unsqueeze(0)                                                            # H,W -> 1,H,W
                pred_masks = torch.cat((bg_mask,pred_masks),dim=0)   
                del bg_mask                                                       # [bg, inst1, inst2, ..]
            ...
            del all_frames, num_interactions_for_sequence, num_interactions_per_instance_for_sequence , instance_level_iou  
...
def load_sequence_images(seq):
    image_root = '/globalwork/roy/dynamite_video/mivos_dynamite/MiVOS_DynaMITe/datasets/MOSE/valid/JPEGImages'
